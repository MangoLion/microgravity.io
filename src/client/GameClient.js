...
class GameClient extends Game {
    ...
    get smallUI() { return this.screenWidth <= 950; }

    get inputFocused() { return document.activeElement instanceof HTMLInputElement; }

    get modalShowing() { return this.showingAlliances || this.showingHelp; }

    get buildTipSlot() { return this.vue.hoveringStructure ? structures.structureForId(this.vue.hoveringStructure).slot : null; }

    get averageFPS() { return this.framesPerSecondHistory.reduce((n, v) => n + v) / this.framesPerSecondHistory.length; }
    ...
    get normalizedUsername() { return this.currentPlayer ? this.currentPlayer.username.toLowerCase() : ""; }
    get isReee() { return this.normalizedUsername.startsWith("ree"); }
    get isLSD() { return this.normalizedUsername === "lsd"; }
    get isFuckNathan() { return this.normalizedUsername === "#fucknathan" || this.normalizedUsername === "#fuckingnathan"; }
    ...
    initUI() {
        ...
        this.vue = new Vue({
            ...
            computed: {
                // General
                timeToKick() { return config.kickTimeout - this.kickTimer; },
                ...
                // Building
                buildableStructures() {
                    return this.structureList.filter(s => (this.isLanded && s.planetItem) || (!this.onPlanet && !s.planetItem));
                },
            ...
            watch: {
                buildableStructures(val) {
                    ...
                    if (this.hoveringStructure && val.findIndex(s => s.id === this.hoveringStructure) === -1) {
                        this.hoveringStructure = null;
                    }
                },
        ...
        if (config.isProd) window.onbeforeunload = () => "Are you sure you want to leave? You will lose all progress.";
        ...
        /* Alliances */
        document.getElementById("showAllianceButton").addEventListener("click", () => this.toggleAlliances(true));
        document.getElementById("dismissAlliances").addEventListener("click", () => this.toggleAlliances(false));
        ...
        document.getElementById("acceptJoinRequestButton").addEventListener("click", () => this.sendResolveJoinRequest(true));
        document.getElementById("denyJoinRequestButton").addEventListener("click", () => this.sendResolveJoinRequest(false));
        ...
        this.controllingTouch = { id: -1, startX: 0, startY: 0, currentX: 0, currentY: 0 };
        this.attackingTouch = { id: -1, startX: 0, startY: 0, currentX: 0, currentY: 0 };
    ...
    initInput() {
        ...
        document.addEventListener("mouseup", e => {  // Even is on document so if mouse is moved out of element and released, it still gets the event
            if (e.button === 0) {
                this.leftClick = false;
            } else if (e.button === 1) {
                this.middleClick = false;
    ...
    sendJoin() {
        ...
        // Join game
        this.send(config.clientMessages.JOIN, [ username, this.vue.selectedShip, this.vue.selectedFill, [...this.rewards.values()] ]);
    }
    ...
    update(dt) {
        ...
        this.vue.ownedWeaponIndex = this.currentPlayer ? this.ownedWeapons.findIndex(w => w[0] === this.currentPlayer.weaponIndex) : -1;
    }
    ...
    updateInput() {
        ...
        let targetRot = this.usingTouch ? (this.serverInputTargetRot || 0) : Math.atan2(this.mouseY - window.innerHeight / 2, this.mouseX - window.innerWidth / 2);  // Use old rot if using touch
        let firing = false;
        let sprinting = false;
        ...
        if (this.rightClick || this.middleClick) {  // Middle click reverses
        ...
        if (this.controllingTouch.id !== -1) {
            let dist = utils.dist(this.controllingTouch.startX, this.controllingTouch.startY, this.controllingTouch.currentX, this.controllingTouch.currentY);
            moveSpeed = dist > 10 ? 1 : 0;
            moveDir = Math.atan2(-(this.controllingTouch.currentY - this.controllingTouch.startY), this.controllingTouch.currentX - this.controllingTouch.startX);
            sprinting = dist > this.touchTriggerThreshold;

        }
        if (this.attackingTouch.id !== -1) {
            let dist = utils.dist(this.attackingTouch.startX, this.attackingTouch.startY, this.attackingTouch.currentX, this.attackingTouch.currentY);
            targetRot = Math.atan2(this.attackingTouch.currentY - this.attackingTouch.startY, this.attackingTouch.currentX - this.attackingTouch.startX);
    ...
    render() {
        ...
        if (settings.spaceDust && !this.isLSD) {
            ...
            ctx.save();
            ctx.fillStyle = ctx.createPattern(assets.getImage(assets.effectUrl("space-texture"), 256, 256), "repeat");
            ctx.globalCompositeOperation = "screen";
            ctx.globalAlpha = 0.1;
            ctx.scale(scale, scale);
            ctx.translate(-this.viewportX * translateMultiplier, -this.viewportY * translateMultiplier);
            ctx.fillRect(this.viewportX * translateMultiplier, this.viewportY * translateMultiplier, this.width / scale, this.height / scale);
            ctx.restore();
        }
    ...
    renderWorld(ctx, dt) {
        ...
        shakeMagnitude = Math.min(shakeMagnitude, settings.rss ? config.maxScreenShakeReduced : config.maxScreenShake);
        ...
        // Draw the entities
        let layers = Object.keys(this.renderLayers).map(l => parseFloat(l)).sort((a, b) => a - b);
        ...
        for (let entityId in this.chatMessages) {
            ...
            if (entity) {
                ...
                entityVisible = x > (this.viewportX - viewWidth / 2) && x < (this.viewportX + viewWidth / 2) &&
                    y > (this.viewportY - viewHeight / 2) && y < (this.viewportY + viewHeight / 2);
            }
            ...
            // Render the message
            if (entityVisible) {  // Render in place
                let x = entity.x;
                ...
                ctx.fillRect(
                    x - (messageWidth + paddingX) / 2, y - (messageHeight + paddingY) / 2,
                    messageWidth + paddingX, messageHeight + paddingY
                );
                ctx.restore();
                ...
                ctx.restore();
            } else if (this.allianceData && this.allianceData[3].findIndex(c => c[0] === clientId) !== -1) {  // Render from afar if in alliance
        ...
        if (config.debugLayer) {
            ...
            ctx.strokeRect(
                -config.viewDistanceX / 2 * this.viewDistanceMultiplier, -config.viewDistanceY / 2 * this.viewDistanceMultiplier,
                config.viewDistanceX * this.viewDistanceMultiplier, config.viewDistanceY * this.viewDistanceMultiplier
            );
            ctx.restore();
        }
    ...
    renderUI(ctx, dt) {
        ...
        if (this.currentPlayer) {
            ...
            // Set the base values
            vignetteColor = vignetteColor.map((v, i) => utils.lerp(v, config.lowHealthVignette[i], 1 - health));
        ...
        this.killVignetteProgress = Math.max(this.killVignetteProgress - 1 / this.killVignetteProgressLength * dt, 0);
        vignetteColor = vignetteColor.map((v, i) => utils.lerp(v, config.killVignette[i], this.killVignetteProgress));
        vignetteDistance = utils.lerp(vignetteDistance, config.killVignetteDistance, this.killVignetteProgress);
        ...
        let diagonalRadius = utils.dist(this.screenWidth / 2, this.screenHeight / 2, this.screenWidth, this.screenHeight);
        let vignetteGradient = ctx.createRadialGradient(this.screenWidth / 2, this.screenHeight / 2, 0, this.screenWidth / 2, this.screenHeight / 2, diagonalRadius);
        ...
        // Draw touch controls
        if (this.controllingTouch.id !== -1) this.renderControl(ctx, this.controllingTouch.startX, this.controllingTouch.startY, this.controllingTouch.currentX, this.controllingTouch.currentY);
        if (this.attackingTouch.id !== -1) this.renderControl(ctx, this.attackingTouch.startX, this.attackingTouch.startY, this.attackingTouch.currentX, this.attackingTouch.currentY);
        ...
        if (config.debugLayer) {
            ...
            ctx.textAlign = "left";
            utils.fillMultilineText(ctx, `Ping: ${Math.round(this.ping * 1000)}ms\nFPS: ${Math.round(this.averageFPS)}`, 20, this.screenHeight / 2);
            ctx.restore();
        }
    }
    ...
    renderStats(ctx) {
        ...
        ctx.fillStyle = "rgba(0, 0, 0, 0.5)";
        ctx.fillRect(this.screenWidth - bgWidth - marginX, this.screenHeight - bgHeight - marginY, bgWidth, bgHeight);
        ctx.restore();
        ...
        for (let i = 0; i < this.resources.length; i++) {
            ...
            // Draw icons
            utils.drawImage(ctx, assets.iconUrl(resourceType.icon), xOffset - textWidth - iconSize, yOffset, iconSize, iconSize);
        ...
        ctx.fillText(Math.floor(this.score), this.screenWidth - textPaddingX, this.screenHeight - textPaddingY);
        ctx.restore();
        ...
        ctx.save();
        ctx.globalAlpha = 0.5;
        ctx.fillRect(this.screenWidth - marginX - padding - fireWidth, this.screenHeight - marginY - padding - fireHeight, fireWidth, fireHeight);
        ctx.restore();
        ...
        ctx.fillRect(this.screenWidth - marginX - padding - sprintWidth - fireWidth, this.screenHeight - marginY - padding - sprintHeight, sprintWidth, sprintHeight);
    ...
    renderMinimap(ctx) {
        ...
        if (this.currentPlayer) {
            ...
            ctx.lineTo(Math.cos(Math.PI + angleSpread) * iconRadius, Math.sin(Math.PI + angleSpread) * iconRadius);
    ...
    renderTextButton(ctx, text, x, y, width = -1, height = -1, paddingX = 10, paddingY = 5, backgroundStyle = null) {
        ...
        return this.renderButton(ctx, x, y, textWidth, textHeight, ctx => {
    ...
    renderButton(ctx, x, y, width, height, renderContent, backgroundStyle = "white") {
        ...
        // Determine state
        let hovering = mouseX < width / 2 && mouseX > -width / 2 &&
    ...
    onUpdate(data) {
        let [playerId, spectateId, appearedEntities, updatedEntities, disappearedEntities, destroyedEntities] = data;
    ...
    onLeaderboard(data) {
        ...
        let itemCount = this.leaderboardData.length + (this.playerRank >= this.leaderboardData.length ? 1 : 0);
        ...
        for (let i = 0; i < itemCount; i++) {
            ...
            let allianceData = allianceId != null ? this.allianceList.filter(a => a[0] === allianceId)[0] : null;
    ...
    onResources(data) {
        ...
        if (this.score > 0 || this.resources.filter(r => r > 0).length > 0) {
            ...
            respawnTip.innerHTML = utils.translate("respawn-tip", utils.generateResourcesHTML(this.resources), Math.floor(this.score));
        } else {
    ...
    updateGameState() {
        ...
        document.getElementById("mainMenu").style.display = (!showingDisconnect && !showingLoading && !inGame) ? "block" : "none";
        document.getElementById("gameOverlay").style.display = (inGame && !this.modalShowing) ? "block" : "none";
    ...
    toggleAlliances(show) {
        ...
        if (show) {
            ...
            document.getElementById("createAllianceHolder").style.display = this.allianceData ? "none": "block";
            document.getElementById("leaveAllianceButton").style.display = this.allianceData ? "block": "none";
        }
    ...
    updateControlPanel() {
        ...
        if (planet) {
            ...
            document.getElementById("planetPanelTitle").innerText = utils.translate("planet-label", planet.name);

            let resourceValues = [];
            for (let i = 0; i < planet.resources.length; i++) {
                let generationRate = planet.resourceGenerationRate[i] - planet.totalResourceDemand[i];
                resourceValues.push(`<span class="${planet.resources[i] < 10 ? "warning" : ""}">${Math.floor(planet.resources[i])}/${planet.resourceMax[i]}</span> (<span class="${generationRate < 0 ? "warning" : ""}">${generationRate.toFixed(1)}/s</span>)`);
            }

            let details = `<div>${utils.translate("citizens")}: ${Math.floor(planet.totalLaborAvailable)}</div><div>${utils.translate("labor-demand")}: <span class="${planet.totalLaborDemand > planet.totalLaborAvailable ? "warning" : ""}">${Math.floor(planet.totalLaborDemand)}</span></div>`;
        ...
        if (structure) {
            ...
            if (structure.canInteract(this.clientId)) {
                if (structure.isMine) {
                    ...
                    addDetails("mineConsumption", `${utils.translate("consumption-label")}:${utils.generateResourceHTML(structure.resourceIndex, Math.floor(structure.adjustedGenerationRate))}/s`);

                    // Mine actions
                    addAction("mineCollectCount", `${utils.translate("collect-label")}${utils.generateResourceHTML(structure.resourceIndex, collectCount)}`, () => {
                        this.sendStructureAction(structures.actions.COLLECT, collectCount);
                    }, canCollect);
                    addAction("mineCollectAll", `${utils.translate("collect-label")}${utils.generateResourceHTML(structure.resourceIndex, utils.translate("all-label"))}`, () => {
                    ...
                    addDetails("factoryResources", utils.generateResourcesHTML(resourceStats, true, false, i => structure.weapon.bulletResources[i] > 0));
                    if (produceCount > 0) {
                    ...
                    for (let i = 0; i < structure.depositedResources.length; i++) {
                        ...
                        let count = Math.max(structure.generationRate * structure.bulletResources[i] * depositDuration, structure.bulletResources[i]);
                    ...
                    // Collect actions
                    addAction("factoryCollectCount", `${utils.translate("collect-label")} ${collectCount} ${structure.weapon.name}`, () => {
                        this.sendStructureAction(structures.actions.COLLECT, collectCount);
                    }, canCollect);
    ...
    pointVisible(x, y, padding = 0) {
        return x < this.viewportX + this.viewWidth / 2 + padding && x > this.viewportX - this.viewWidth / 2 - padding &&
            y < this.viewportY + this.viewHeight / 2 + padding && y > this.viewportY - this.viewHeight / 2 - padding;
    }
}
