...
class Game {
    ...
    _removeEntity(entity) {
        ...
        if (entity == null || !this.entities[entity.id]) throw new Error(`Entity for ID ${entity.id} does not exist.`);
        ...
        if (config.isClient) {
            for (let layerDepth of entity.renderLayers) {
                ...
                if (index !== -1) {
                    ...
                    console.error(`Failed to find entity ${entity.id}'s render layer ${entity.renderLayers}.`);
                }
            }
        }
