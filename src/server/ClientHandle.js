...
class ClientHandle {
    ...
    sendUpdate() {
        ...
        this.game.queryRect(spectateX, spectateY, config.viewDistanceX * this.viewDistanceMultiplier, config.viewDistanceY * this.viewDistanceMultiplier, entity => {
        ...
        // Send data
        let data = [playerId, spectateId, this.transformEntityMap(appearedEntities), this.transformEntityMap(updatedEntities), disappearedEntities, destroyedEntities];
        ...
        for (let entityId in this.game.entities) {
            ...
            if (shouldAdd) {
                ...
                if (entity.isPlayer && entity.clientHandle) {
                    extraData = [entity.clientHandle.id, entity.clientHandle.alliance && entity.clientHandle.alliance.id];
                ...
                // Add base data
                minimapItems.push([
                    entity.constructor.KIND, entity.id,
                    utils.quantize(entity.x, -mapRadius, mapRadius, config.minimapPositionBits), utils.quantize(entity.y, -mapRadius, mapRadius, config.minimapPositionBits),
                    utils.quantize(entity.bodyRadius || entity.radius, 0, config.minimapMaxRadius, config.minimapRadiusBits),
                    extraData
                ]);
            }
        }
    ...
    sendResourceTextPopup(type, count, x, y) {
        this.sendTextPopup(`${Math.floor(count)}`, x, y, assets.iconUrl(resources.resources[type].id), count / 200);
    }

    sendMessage(entity, text) {
        this.send(config.serverMessages.MESSAGE, [entity.id, entity.clientHandle ? entity.clientHandle.id : null, entity.username, entity.x, entity.y, text]);
    }
    ...
    sendUpgradeOptions() {
        this.send(config.serverMessages.UPGRADE_OPTIONS, [this.rankFloor > this.upgradedRank ? this.upgradedRank : null, this.getPerkIndexList()]);
    }
    ...
    onInit(data) {
        ...
        if (challengeResponse !== correctChallengeResponse) {
            console.warn(`Client ${this.id} answered challenge incorrectly. Guessed ${challengeResponse}, correct answer was ${correctChallengeResponse}.`);
            this.ws.close();
            return;
        }
    ...
    onJoin(data) {
        ...
        if (!foundSpawnPosition) [this.player.x, this.player.y] = this.game.chooseSpawnPoint(this.player.radius, 300);
    ...
    onPurchaseStructure(index, force = false) {
        ...
        if (this.structureCounts[structureData.slotIndex] && this.structureCounts[structureData.slotIndex] >= structureData.limit) {
            this.sendNotification(config.notificationTypes.TOO_MANY_STRUCTURES, "notification-too-many-structures");
            return;
        }
        ...
        let collides = false;
        this.game.queryCircle(structure.x, structure.y, structure.structure.mineRadius || structure.radius, entity => {
            if (entity.isStructure) collides = true;
        });
        if (collides) {
            this.sendNotification(config.notificationTypes.STRUCTURE_PLACE_CONFLICT, "notification-structure-place-conflict");
            return;
        }
        ...
        if (structure.structure.id === "wall") {
            ...
            if (nearWall) {
                this.sendNotification(config.notificationTypes.STRUCTURE_PLACE_CONFLICT, "notification-near-wall");
                return;
            }
        }
        ...
        if (text.startsWith("/")) {
            ...
            switch (command) {
                ...
                case "sign":
                    ...
                    // Create the sign
                    let signIndex = structures.structureIndexForId(this.player.overlappingPlanet ? "planet-sign" : "space-sign");
        ...
        switch (action) {
            ...
            case structures.actions.COLLECT:
                ...
                if (structure.isMine) {
                    ...
                    this.giveResources(resourceIndex, count, structure.x - Math.cos(structure.rot + Math.PI / 2) * structure.radius * 0.5, structure.y + Math.sin(structure.rot + Math.PI / 2) * structure.radius * 0.5);
                    this.sendResources();
    ...
    onChooseUpgrade(index) {
        ...
        if (upgrade.structure) {
            ...
            let replaceIndex = this.ownedStructures.findIndex(i => structures.structures[i].slot === upgrade.structure.slot);
            if (replaceIndex !== -1) {
    ...
    killedPlayer(victim) {
        ...
        // Send notification
        this.sendNotification(config.notificationTypes.PLAYER_KILL, "notification-player-kill", [utils.killerLabel(victim)]);
    }
    ...
    giveResources(type, count, x = null, y = null, multiple = false) {
        ...
        let randomSpread = 75;
        if (x != null && y != null) this.sendResourceTextPopup(type, count, multiple ? utils.randomInRange(x - randomSpread, x + randomSpread) : x, multiple ? utils.randomInRange(y - randomSpread, y + randomSpread) : y);
        this.sendResources();
    }
    ...
    giveAmmo(weaponIndex, ammo) {
        ...
        if (ammo <= 0) return;
        let maxAmmo = this.player ? this.player.getMaxAmmo(weaponIndex) : weapons.weapons[weaponIndex].maxAmmo;
