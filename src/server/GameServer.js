...
class GameServer extends Game {
    ...
    generatePlanets() {
        ...
        centerPlanet.resourceGenerationRate = centerPlanet.resourceGenerationRate.map(() => config.centerPlanetResourceGenRate);
        ...
        for (let i = 0; i < planetCount; i++) {
            ...
            let resourceGenRateMultiplier = (1 / planet.resourceGenerationRate.reduce((t, n) => t + n)) * totalGenRate;
            planet.resourceGenerationRate = planet.resourceGenerationRate.map(v => v * resourceGenRateMultiplier);
    ...
    broadcastLeaderboard() {
        ...
        let leaderboard = this.clients
            ...
            .map(client => {
                return [client.id, client.alliance ? client.alliance.id : null, client.username, client.score];
            });
    ...
    generateAllianceData() {
        ...
        for (let allianceId in this.alliances) {
            ...
            alliances.push([alliance.id, alliance.owner && alliance.owner.id, alliance.name, alliance.clients.length, alliance.clients.map(c => c.id)]);
        }
