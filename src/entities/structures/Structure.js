...
class Structure extends Entity {
    ...
    get adjustedMaxHealth() { return this.isConstructed ? this.maxHealth : this.constructionHealth; }
    get constructionProgress() { return 1 - this.constructionTimer / config.structureConstructionTime; }

    get showBuildTips() {
        return this.game.buildTipSlot === this.structure.slot &&  // Hovering on a structure that matches this slot
            this.game.currentPlayer && this.game.currentPlayer.isFriendlyClientId(this.clientOwnerId);  // This is a friendly structure
    }
    ...
    onCollision(entity, dt) {
        if (entity.isPlayer || entity.isAsteroid) {
            ...
            if (entity.shooterEntity && entity.shooterEntity.isPlayer) utils.damageEntity(entity, this, entity.damage * damageMultiplier);
        }
    }
    ...
    onKilled(killer) {
        if (config.isServer) {
            ...
            if (destroyerLabel) {
                ...
                let notificationLabel = this.hostPlanet ? "notification-structure-destroyed-planet" : "notification-structure-destroyed";
                let notificationData = this.hostPlanet ? [destroyerLabel, structureName, this.hostPlanet.name] : [destroyerLabel, structureName];
                this.clientOwner.sendNotification(config.notificationTypes.STRUCTURE_DESTROYED, notificationLabel, notificationData);
            }
        }
    }
    ...
    drawImage(ctx, scale = 1) {
        ...
        utils.drawImage(ctx, assets.structureUrl(this.structure.image), 0, 0, this.radius * 2 * scale, this.radius * 2 * scale);
        ctx.restore();
    }
    ...
    render(ctx) {
        ...
        if (this.structure.planetItem) {
            utils.drawGlow(ctx, 0, 0, this.radius * 2, this.ownerAllianceId ? utils.getAllianceColor(this.ownerAllianceId) : null, this.ownerAllianceId ? 0.2 : 0.07);
        }
