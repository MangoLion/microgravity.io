...
class Wall extends Structure {
    get isStrong() { return this.structure.id === "wall-strong"; }
    ...
    update(dt) {
        ...
        for (let [id, x, y] of this.connectedPositions) {
            ...
            this.game.queryMinMax(
                Math.min(this.x, x), Math.max(this.x, x),
                Math.min(this.y, y), Math.max(this.y, y),
                entity => {
                    // Don't collide with walls, friendly players, or bullets jumping walls
                    if (entity.isWall || entity.isPlanet || entity.isWormhole || (entity.isPlayer && entity.isFriendlyClientId(this.clientOwnerId)) || (entity.isBullet && entity.jumpsWalls)) return;

                    // Don't collide with structures
                    if (entity.isStructure) return;

                    // Check that collides
                    let circleCollides = entity.checkCircle && utils.testCircleLine(entity.x, entity.y, entity.radius, this.x, this.y, x, y);
                    let trailCollides = entity.checkTrail && utils.testLineLine(this.x, this.y, x, y, entity.prevX, entity.prevY, entity.x, entity.y);
                    ...
                    let sideSign = Math.sign((x - this.x) * (entity.prevY - this.y) - (y - this.y) * (entity.prevX - this.x));
    ...
    onFinishedConstruction() {
        ...
        if (this.structure.planetItem) {
            ...
            this.connectedPositions = [
                [null, this.x - Math.cos(this.rot + Math.PI / 2) * wallSize, this.y + Math.sin(this.rot + Math.PI / 2) * wallSize]
            ];
        } else {
            // Connect to walls
            this.reevaluateConnectedWalls();
    ...
    reevaluateConnectedWalls() {
        ...
        this.game.queryCircle(this.x, this.y, config.maxWallConnectRadius, wall => {
            ...
            if (!wall.clientOwner || !this.clientOwner || !wall.clientOwner.isFriendly(this.clientOwner.id)) return;  // Make sure the owner is friendly
    ...
    render(ctx) {
        ...
        let playerDist = this.game.currentPlayer && utils.dist(this.x, this.y, this.game.currentPlayer.x, this.game.currentPlayer.y);
        if (
            ...
            playerDist <= config.maxWallConnectRadius && playerDist > config.minWallConnectRadius  // Show within distance
        ) {
        ...
        for (let [id, x, y] of this.connectedPositions) {
            ...
            ctx.lineTo(x - this.x - Math.cos(angle) * lineStartRadius, -(y - this.y - Math.sin(angle) * lineStartRadius));
            ctx.stroke();
            ctx.restore();
        }
...
Wall.addSyncKeys(
    {
        ...
        update: true,
        sendMap: v => v.map(w => [w[0], utils.quantize(w[1], -mapRadius, mapRadius, 16), utils.quantize(w[2], -mapRadius, mapRadius, 16)]),  // Quantize positions
        receiveMap: v => v.map(w => [w[0], utils.unquantize(w[1], -mapRadius, mapRadius, 16), utils.unquantize(w[2], -mapRadius, mapRadius, 16)]),  // Unquantize positions
    }
);
