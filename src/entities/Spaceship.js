...
class Player2 extends Entity {
    ...
    get clientId() { return this.clientHandle ? this.clientHandle.id : this._clientId; }
    set clientId(id) { this._clientId = id; }

    get maxSprint() { return config.sprintTime * this.getPerkMultiplier("boostAmount"); }
    get fireInterval() {
        let weapon = this.weapon;
        return weapon.fireInterval * this.getPerkMultiplier(weapon.normalizedId + "Interval");
    }

    get maxAmmo() { return this.getMaxAmmo(this.weaponIndex) }
    ...
    update(dt) {
        ...
        if (config.isServer && (this.isLanded || (Date.now() - this.lastDamageTime) / 1000 >= this.healWaitTime)) {
        ...
        let thrustMultiplier = this.moveSpeed * (isSprinting ? (config.playerSprintingAcceleration * this.getPerkMultiplier("boostSpeed")) : (config.playerAcceleration * this.getPerkMultiplier("moveSpeed"))) * dt;
        // if (this.isBot && !this.overlappingPlanet) thrustMultiplier *= 0.6;
        this.addVelocity(
            Math.cos(thrustAngle) * thrustMultiplier,
            Math.sin(thrustAngle) * thrustMultiplier
        );
        ...
        if (this.useDrag) {
            ...
            if (this.moveSpeed < 0.1) {
                this.addVelocity(
                    -this.velX * config.playerDrag * dt,
                    -this.velY * config.playerDrag * dt
                );
            }
        } else {
        ...
        if (this.useMaxVel) {
            ...
            let maxSpeed = isSprinting ? (config.playerMaxSprintVelocity * this.getPerkMultiplier("boostSpeed")) : (config.playerMaxVelocity * this.getPerkMultiplier("moveSpeed"));
            if (this.overlappingPlanet) maxSpeed *= 2.5;
            if (velMagnitude > maxSpeed) {
                velMagnitude = utils.lerp(velMagnitude, maxSpeed, 3 * dt);
            }
            this.setVelocity(
                Math.cos(velAngle) * velMagnitude,
                Math.sin(velAngle) * velMagnitude
            );
        } else {
        ...
        if (config.isClient) {
            ...
            this.thrustVolume = utils.lerp(this.thrustVolume, isThrusting && !isSprinting ? maxVolume : 0, lerpSpeed * dt);
            this.thrustSprintVolume = utils.lerp(this.thrustSprintVolume, isThrusting && isSprinting ? maxVolume : 0, lerpSpeed * dt);
        ...
        while (this.fireTimer <= 0 && attemptFire) {  // Fire enough bullets to make up for the lost bullets in the last tick
            this.fireBullet();
        }
    ...
    onKilled(killer) {
        ...
        if (this.clientHandle && killerLabel) {
            this.clientHandle.sendNotification(config.notificationTypes.DEATH, "notification-death", [killerLabel]);
        }
    }
    ...
    fireBullet() {
        ...
        if (kickback) {
            this.addVelocity(Math.cos(this.rot + Math.PI) * kickback, -Math.sin(this.rot + Math.PI) * kickback);
        }
    }
    ...
    damageFromCollision(source) {
        ...
        let damageAmount = utils.dist(source.velX * sourceVelocityBias, source.velY * sourceVelocityBias, this.velX, this.velY) * config.playerDamagePerVelocity;
    ...
    collisionPerkDamage(target) {
        ...
        if (target.health != null) {
            ...
            let damageAmount = utils.dist(this.velX, this.velY, target.velX, target.velY) * collisionDamagePerVelocity;
    ...
    isFriendlyClientId(clientId) {
        ...
        if (config.isClient) {
            ...
            if (this.allianceId) {
                let allianceData = this.game.allianceList[this.game.allianceList.findIndex(a => a[0] === this.allianceId)];
    ...
    render(ctx, dt) {
        ...
        /* Draw glow */
        utils.drawGlow(ctx, this.x, this.y, this.radius * 6, this.allianceId ? utils.getAllianceColor(this.allianceId) : null, this.allianceId ? 0.3 : 0.1);
        ...
        if (this.isLanded) {
            let planetAngle = Math.atan2(this.y - this.overlappingPlanet.y, this.x - this.overlappingPlanet.x);
        ...
        // Render name
        let alliance = this.game.allianceList && this.game.allianceList[this.game.allianceList.findIndex(a => a[0] === this.allianceId)];
    ...
    drawFire(ctx, fireDepth, fireWidth) {
        ...
        ctx.bezierCurveTo(
            -fireWidth / 2, 0,  // Left control point
            -controlPointDistance, fireDepth,  // Bottom control point
            0, fireDepth  // Bottom position
        );
        ctx.bezierCurveTo(
            controlPointDistance, fireDepth,  // Bottom control point
            fireWidth / 2, 0,  // Right control point
            fireWidth / 2, 0  // Right position
        );
        ctx.fill();
    }
...
Player2.addSyncKeys(
    ...
    { key: "moveDir", type: "float", update: true, sendMap: v => utils.modFix(v, Math.PI * 2) },
    { key: "moveSpeed", type: "float", update: true, quantize: config.quantize01 },
    { key: "relativeMoveDir", type: "boolean", update: true },
    { key: "targetRot", type: "float", update: true, sendMap: v => utils.modFix(v, Math.PI * 2) },
    { key: "firing", type: "boolean", update: true },
    { key: "sprinting", type: "boolean", update: true },
    { key: "weaponIndex", type: "int", update: true },
    { key: "shieldTimer", type: "float", update: true, sendMap: v => utils.quantizeSimple(v, 1), receiveMap: v => utils.unquantizeSimple(v, 1) },
    { key: "sprint", type: "float", update: true, quantize: { min: 0, max: config.sprintTime * 4, bits: 8 } }
);
