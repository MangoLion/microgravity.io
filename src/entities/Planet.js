    ...
    update(dt) {
        ...
        if (config.isServer) {
            ...
            if (config.isServer && this.game.updateIndex % 5 === 0) {
                ...
                this.game.queryCircle(this.x, this.y, this.radius, entity => {
                    if (entity.isStructure && entity.isConstructed) {
                        if (entity.isCity) {
                            ...
                            let key = clientOwner.alliance ? `a:${clientOwner.alliance.id}` : `p:${clientOwner.id}`;
                ...
                let dominatingItem = Object.keys(dominationCities).sort((a, b) => dominationCities[b] - dominationCities[a])[0];
        ...
        if (layerDepth === this.renderLayers[0]) {
            ...
            if (settings.planetProps) {
                ...
                for (let i = 0; i < humanCount; i++) {
                    ...
                    ctx.scale(moveDir, 1);
                    utils.drawImage(ctx, assets.propUrl(isVehicle ? "vehicle-1" : `human-${humanIndex}`), 0, 0, humanSize, humanSize);
                    ctx.restore();
                }
            }
...
Planet.addSyncKeys(
    ...
    { key: "atmosphereSize", type: "float", update: false, quantize: { min: 0, max: config.centerPlanetAtmosphereSize, bits: 7 } },
    { key: "resources", type: "int[]", update: true, sendMap: v => v.map(r => Math.floor(r)) },
    { key: "resourceMax", type: "int[]", update: false, sendMap: v => v.map(r => Math.floor(r)) },
    { key: "resourceGenerationRate", type: "float[]", update: false, sendMap: v => utils.quantizeSimpleArray(v, 1), receiveMap: v => utils.unquantizeSimpleArray(v, 1) },
    { key: "totalLaborAvailable", type: "int", update: true, sendMap: v => Math.floor(v) },  // Round down
    { key: "totalLaborDemand", type: "int", update: true, sendMap: v => Math.floor(v) },  // Round down
    { key: "totalResourceDemand", type: "float[]", update: true, sendMap: v => utils.quantizeSimpleArray(v, 1), receiveMap: v => utils.unquantizeSimpleArray(v, 1) }
);
