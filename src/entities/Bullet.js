...
class Bullet extends Entity {
    ...
    // Properties
    get bulletCheckCircle() { return this.bulletPrefab.checkCircle || false; }
    get bulletRadius() { return this.bulletPrefab.radius || 4; }
    get bulletSpeed() { return this.bulletPrefab.bulletSpeed || 0; }
    get bulletDrag() { return this.bulletPrefab.bulletDrag || 0; }
    get bulletAcceleration() { return this.bulletPrefab.bulletAcceleration || 0; }
    get bulletAccelerationDelay() { return this.bulletPrefab.bulletAccelerationDelay || 0; }
    get damage() { return (this.bulletPrefab.damage || 0) * this.getPerkMultiplier(this.bulletPrefab.normalizedId + "Damage"); }
    get life() { return (this.bulletPrefab.life || 0.8) * this.getPerkMultiplier(this.bulletPrefab.normalizedId + "Life"); }

    // Explosion
    get explosionRadius() { return (this.bulletPrefab.explosionRadius || 0) * this.getPerkMultiplier("explosionRadius"); }
    get explosionSpeed() { return this.bulletPrefab.explosionSpeed || 80; }
    get explosionDamage() { return this.bulletPrefab.explosionDamage || 0; }
    get explosionVelocity() { return this.bulletPrefab.explosionVelocity || 0; }

    // Rendering
    get renderType() { return this.bulletPrefab.renderType || "trail"; }
    get trailLength() { return this.bulletPrefab.trailLength || 20; }
    get blinkInterval() { return this.bulletPrefab.blinkInterval || -1; }
    get pulseInterval() { return this.bulletPrefab.pulseInterval || -1; }
    get renderIcon() { return this.bulletPrefab.renderIcon || null; }
    get renderColor() { return this.bulletPrefab.renderColor || "white"; }

    // Sound
    get shootSound() { return this.bulletPrefab.id || "machine-gun"; }
    get shootVolume() { return this.bulletPrefab.volume || 1.0; }

    // Client
    get shooterEntityId() { return this.shooterEntity ? this.shooterEntity.id : this._shooterEntityId; }
    set shooterEntityId(id) { this._shooterEntityId = id; }
    get shooterClientId() { return this.shooterClient ? this.shooterClient.id : this._shooterClientId; }
    set shooterClientId(id) { this._shooterClientId = id; }
    ...
    update(dt) {
        ...
        // Apply drag
        this.addVelocity(
            -this.velX * this.bulletDrag * dt,
            -this.velY * this.bulletDrag * dt
        );
    ...
    destroyBullet() {
        ...
        if (config.isClient) {
            if (this.explosionRadius) {
                ...
                this.game.explosionManager.spawnExplosion(utils.lerp(this.x, this.prevX, 0.5), -utils.lerp(this.y, this.prevY, 0.5), this.explosionRadius, this.explosionSpeed);
            } else {
                // Show basic explosion
                this.game.explosionManager.spawnExplosion(utils.lerp(this.x, this.prevX, 0.5), -utils.lerp(this.y, this.prevY, 0.5), 15);
            }
        }

        // Damage entities
        if (config.isServer && this.explosionRadius) {
            this.game.spawnExplosion(this.x, this.y, this.explosionRadius, this, this.explosionDamage, this.explosionVelocity, e => {
                ...
                if (clientOwner && this.shooterClient && this.shooterClient.isFriendly(clientOwner.id)) return false;  // Don't damage friendlies
    ...
    render(ctx, dt) {
        ...
        if (this.renderType === "trail" || (this.renderType === "triangle" && this.bulletAccelerationTimer <= 0)) {
            // Draw trail
            ...
            utils.drawImage(ctx, assets.effectUrl("gradient-trail"), -imageTrailLength / 2, 0, imageTrailLength, imageTrailWidth);
            ctx.restore();
        }
        ...
        switch (this.renderType) {
            ...
            case "triangle":
                ...
                ctx.lineTo(Math.cos(Math.PI + angleSpread) * this.radius, Math.sin(Math.PI + angleSpread) * this.radius);
                ctx.lineTo(Math.cos(Math.PI - angleSpread) * this.radius, Math.sin(Math.PI - angleSpread) * this.radius);
                ctx.fill();
        ...
        // Render icon
        if (this.renderIcon) {
            utils.drawImage(ctx, assets.iconUrl(this.renderIcon), 0, 0, this.bulletRadius * 1.5, this.bulletRadius * 1.5);
        }
    }
}
